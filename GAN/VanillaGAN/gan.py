...
class GAN():
	def __init__(self):
		...
		#only train the generator for stacked model
		self.discriminator.trainable = False 

		#discriminator takes the generated images as input and determines reality
		reality = self.discriminator(image)

		#stackedmodel: Takes in random noise as input, spits out reality as output 
		# noise---> [Generator] --->  image ---> [Discriminator] ---> reality(percentage)
		#trains the generator to fool the discriminator 
		self.stacked= Model(noise_in, reality)
		self.stacked.compile(loss='binary_crossentropy', optimizer = opt)
	...
	def build_generator(self):
		...
		dense1 = Dense(256)(z)
		lrl1 = LeakyReLU(alpha=0.2)(dense1)
		bn1 = BatchNormalization(momentum=0.8)(lrl1)
		dense2 = Dense(512)(bn1)
		lrl2 = LeakyReLU(alpha=0.2)(dense2)
		bn2 = BatchNormalization(momentum=0.8)(lrl2)
		dense3 = Dense(1024)(bn2)
		lrl3 = LeakyReLU(alpha=0.2)(dense3)
		bn3 =  BatchNormalization(momentum=0.8)(lrl3)
		dense4 = Dense(np.prod(self.image_shape), activation='tanh')(bn3)
		image = Reshape(self.image_shape)(dense4)
		model = Model(z,image)
		model.summary()
	...
	def build_discriminator(self):
		...
		dense1 = Dense(512, activation=LeakyReLU(alpha=0.2))(flatten)
		dense2 = Dense(256, activation=LeakyReLU(alpha=0.2))(dense1)
	...
	def train(self, epochs=100, batch_size=128, sample_interval=50):
		...
		for epoch in range(epochs):
			...
			i = np.random.randint(0, X_train.shape[0], batch_size) #returns array sized batch_size with randomly sampled integers
			...
			g_loss = self.stacked.train_on_batch(noise,real)
